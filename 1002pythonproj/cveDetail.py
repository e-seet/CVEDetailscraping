...
def CveDetails(linkList, year):
    ...
    for link in linkList:
        ...
        accessComplexity = text[text.find('Access Complexity') + len('Access Complexity') + 1:text.find('Authentication') - 3]
        ...
        if cvssScore < 0.1:
            cvssScoreCat = 'None'
        elif 0.1 <= cvssScore <= 3.9:
            cvssScoreCat = 'Low'
        elif 4.0 <= cvssScore <= 6.9:
            cvssScoreCat = 'Medium'
        elif 7.0 <= cvssScore <= 8.9:
            cvssScoreCat = 'High'
        elif 9.0 <= cvssScore <= 10.0:
            cvssScoreCat = 'Critical'
        ...
        for i in vulnList:
            ...
            cveDetail.append(accessComplexity[:accessComplexity.find('\n')])
            ...
            if i != '':
                ...
                cveDetail.append('-')
            cveDetail.append(int(year))
            cveDetail.append(cvssScoreCat)
...
def writeToCSV(year, cveDetails, affectedProducts):
    header1 = ["CVE ID", 'Link to CVE', 'CVSS_Score', 'Confidentiality Impact', 'Integrity Impact',
              'Availability Impact', 'Access Complexity', 'Authentication', 'Gained Access', 'Vulnerability Type(s)',
               'Year', 'CVSS Score Category']
...
def main():
    for year in numOfpages:
        ...
        cveDetails, affectedProducts = CveDetails(linkList, year)
